...
function Calculate() {
    ...
    if (document.getElementById(helperK)){
        ...
        if (checkPairs(theBigData) == false){
            if (language == "en"){
                document.getElementById("error_text").innerHTML = "This analysis presumes measurements of the same data points (i.e., participants, instances, etc.) and therefore your data sets should have the same numbers of values, but yours do not. Please check, amend as necessary and retry.";
                document.getElementById('explain_bun').innerHTML = "An error has ocurred. Please see the error message above.";
            } else if (language == "jp"){
                document.getElementById("error_text").innerHTML = "この分析をする際、全ての変数には同じデータの数が必要ですが、入力したデータに相違があります。データの数を確認した上で、もう一度、計算ボタンを押してみてください。";
...
function calculateForReal(k, data){
    ...
    let total = sum(pqs);
    let thisVariance = varianceP(yokos);
    var alpha = (k/(k-1)) * (1 - total/thisVariance);
    alpha = alpha.toFixed(2);
    if (language == "en"){
        details_of_test = "Kuder and Richardson Formula 21 was used to test for internal reliability of the data. ";
        if (alpha > 0.69) {
        ...
        details_of_test = "内的整合性を評価するのに、KuderとRichardsonのFormula 20信頼係数を計算した。";
        if (alpha > 0.69) {
