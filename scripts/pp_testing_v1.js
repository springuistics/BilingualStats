...
function Calculate() {
        ...
        if (checks.normal == false || leveneCheck.pValue<.05){
            ...
            if (language == "en") {
                if (checks.normal == false && leveneCheck.pValue<.05){
                    ...
                    details_of_test = "Despite the continuous and normal* nature of your data, a Levene's test revealed the groups' variances were too different from each other (<i>w</i> = "+leveneCheck.wStatistic.toFixed(2)+"; <i>p</i> = "+leveneCheck.pValue.toFixed(2)+"). Therefore the data was treated as ordinal and a Aligned Rank Transformation Test was used was used to check for a significant interaction between treatment type (i.e., the difference between the experiment and control/comparison group) and treatment (i.e., the difference before and after the treatment).";
                }  
            } else if (language == "jp"){
                if (checks.normal == false && leveneCheck.pValue<.05){
                    details_of_test = "本データは連続データですが、正規性の検定の結果*によると、いずれか（あるいは両方）のデータセットがノンパラメトリックとみなされました。また、ルビーン検定の結果によると、データセット間の分散の均質性が不十分でした（<i>w</i> = "+leveneCheck.wStatistic.toFixed(2)+"; <i>p</i> = "+leveneCheck.pValue.toFixed(2)+"）。従って、ARTテスト（Aligned Rank Transformation検定）を利用して、グループ（i.e., 実験群と対照群の区別）とトリートメント（実験前と実験後）の間に交互作用があるかどうかを検証した。";
                } else if (checks.normal == false){
                    ...
                    details_of_test = "本データは連続データであり、正規性が確認*できましたが、ルビーン検定の結果によると、データセット間の分散の均質性が不十分でした（<i>w</i> = "+leveneCheck.wStatistic.toFixed(2)+"; <i>p</i> = "+leveneCheck.pValue.toFixed(2)+"）。従って、ARTテスト（Aligned Rank Transformation検定）を利用して、グループ（i.e., 実験群と対照群の区別）とトリートメント（実験前と実験後）の間に交互作用があるかどうかを検証した。";
                }    
            }
            var superdata = [];
            ...
            var full_eta_A = SS_A / SST;
            var full_eta_B = SS_B / SST;
            var full_eta_int = SS_AxB / SST;
            full_eta_A = full_eta_A.toFixed(2);
            full_eta_B = full_eta_B.toFixed(2);
            full_eta_int = full_eta_int.toFixed(2);
    ...
    if (language == "en"){
        ...
        if (eta_int<.06) {
            results4 = ", with a small effect size; <i>η<sup>2</sup></i> = "+full_eta_int+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_int;
        } else if (eta_int<0.138) {
            results4 =  ", with a medium effect size; <i>η<sup>2</sup></i> = "+full_eta_int+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_int;
        } else if (eta_int>=0.138) {
            results4 =  ", with a large effect size; <i>η<sup>2</sup></i> = "+full_eta_int+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_int;
        }
        ...
        if (eta_A<.06) {
            result3 = ", with a small effect size; <i>η<sup>2</sup></i> = "+full_eta_A+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_A + ", and ";
        } else if (eta_A<0.138) {
            result3 =  ", with a medium effect size; <i>η<sup>2</sup></i> = "+full_eta_A+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_A + ", and ";
        } else if (eta_A>=0.138) {
            result3 =  ", with a large effect size; <i>η<sup>2</sup></i> = "+full_eta_A+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_A + ", and ";
        }
        ...
        if (eta_B<.06) {
            result6 = ", with a small effect size; <i>η<sup>2</sup></i> = "+full_eta_B+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_B + ".";
        } else if (eta_B<0.138) {
            result6 =  ", with a medium effect size; <i>η<sup>2</sup></i> = "+full_eta_B+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_B + ".";
        } else if (eta_B>=0.138) {
            result6 =  ", with a large effect size; <i>η<sup>2</sup></i> = "+full_eta_B+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_B + ".";
        }
    } else if (language == "jp"){
        if (pofInt < .01) {
            if (isNormal==true){
        ...
        if (eta_int<.06) {
            results4 = "。また、 交互作用の効果は小さかった： <i>η<sup>2</sup></i> = "+full_eta_int+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_int;
        } else if (eta_int<0.138) {
            results4 =  "。また、 交互作用の効果は中ぐらいだった： <i>η<sup>2</sup></i> = "+full_eta_int+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_int;
        } else if (eta_int>=0.138) {
            results4 =  "。また、 交互作用の効果は大きかった： <i>η<sup>2</sup></i> = "+full_eta_int+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_int;
        }
        ...
        if (eta_A<.06) {
            result3 = "）、効果量は小さい： <i>η<sup>2</sup></i> = "+full_eta_A+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_A;
        } else if (eta_A<0.138) {
            result3 =  "）、効果量は中ぐらい： <i>η<sup>2</sup></i> = "+full_eta_A+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_A;
        } else if (eta_A>=0.138) {
            result3 =  "）、効果量は大きい： <i>η<sup>2</sup></i> = "+full_eta_A+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_A;
        }
        ...
        if (eta_B<.06) {
            result6 = "）、効果量は小さい：<i>η<sup>2</sup></i> = "+full_eta_B+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_B + "。";
        } else if (eta_B<0.138) {
            result6 =  "）、効果量は中ぐらい：<i>η<sup>2</sup></i> = "+full_eta_B+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_B + "。";
        } else if (eta_B>=0.138) {
            result6 =   "）、効果量は大きい：<i>η<sup>2</sup></i> = "+full_eta_B+", <i>η<sub>p</sub><sup>2</sup></i> = " + eta_B + "。";
        }
    }
